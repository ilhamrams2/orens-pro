...
<div class="space-y-6">
    <x-ui.header title="Anggota" subtitle="Kelola semua anggota dan nilai performa mereka.">
        ...
        <form action="{{ route('users.purge-members') }}" method="POST" class="inline-block" onsubmit="return confirm('PERINGATAN BAHAYA!\n\nApakah Anda yakin ingin menghapus SEMUA akun Member dan Pengurus?\nSemua data akun member & pengurus serta riwayat presensinya akan dihapus permanen.\n\nTindakan ini TIDAK DAPAT DIBATALKAN!')">
            @csrf
            @method('DELETE')
            <x-ui.button variant="outline" size="sm" type="submit" class="!text-red-600 !border-red-200 hover:!bg-red-50">
                <x-slot name="icon">
