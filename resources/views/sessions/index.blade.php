...
@section('content')
<div class="space-y-6">
    <form id="reportForm" action="{{ route('reports.multi') }}" method="POST" target="_blank">
        @csrf
        <div class="flex justify-between items-center">
            ...
            <div class="flex gap-3">
                <button type="submit" class="bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold hover:bg-indigo-700 transition-all flex items-center gap-2 text-sm shadow-lg shadow-indigo-200">
                        ...
                        @forelse($sessions as $s)
                            <tr class="hover:bg-gray-25 transition-all">
                                <td class="px-8 py-5 text-center">
                                    <input type="checkbox" name="session_ids[]" value="{{ $s->id }}" class="rounded text-orens focus:ring-orens border-gray-300 session-checkbox">
                                </td>
                                <td class="px-8 py-5">
